...
public class LineIteratorTest {
	...
	public void testLineIterator() {
		...
		LineIterator lI = new LineIterator(sf, new Pos(2,0), new Pos(1,0)); //Er läuft nach rechts
		while(lI.hasNext()) {
			result.add(lI.next());
		}
		// Spalte:               0  1  2  3
		...
		result.clear();
		lI = new LineIterator(sf, new Pos(3,1), new Pos(-1,0));  //links
		while(lI.hasNext()) {
			result.add(lI.next());
		}
		...
		result.clear();
		lI = new LineIterator(sf, new Pos(0,3), new Pos(0,1));  //oben
		while(lI.hasNext()) {
			result.add(lI.next());
		}
		...
		result.clear();
		lI = new LineIterator(sf, new Pos(0,0), new Pos(0,-1));  //unten
		while(lI.hasNext()) {
			result.add(lI.next());
		}
		Assert.assertTrue(result.contains(new Pos(0,0)));
		...
		lI = new LineIterator(sf, new Pos(1,2), new Pos(1,1));  //oben rechts
		while(lI.hasNext()) {
			result.add(lI.next());
		}
		...
		Assert.assertFalse(result.contains(new Pos(3,0)));
		result.clear();
		lI = new LineIterator(sf, new Pos(3,0), new Pos(-1,-1));  //unten links
		while(lI.hasNext()) {
			result.add(lI.next());
		}
		Assert.assertTrue(result.contains(new Pos(2,1)));
